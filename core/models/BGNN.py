...
class BGNN(BaseModel):
    def __init__(self,
                 task='classification', iter_per_epoch=10, lr=0.01, hidden_dim=64, dropout=0.,
                 only_gbdt=False, train_non_gbdt=False,
                 name='GB_HGNN', use_leaderboard=False, depth=6, gbdt_lr=0.1):
        super(BaseModel, self).__init__()  # 清空之前的__init__程序
    ...
    def fit(self, networkx_graph, X, y, train_mask, val_mask, test_mask, cat_features,
        ...
        # initialize for early stopping and
        ...
        if self.lang == 'dgl':
            ...
            graph = self.networkx_to_torch3(networkx_graph)

        self.init_gnn_model(graph)
        ...
        for epoch in pbar:
            ...
            loss = self.train_and_evaluate(model_in, y, train_mask, val_mask, test_mask,
                                           optimizer, metrics, self.iter_per_epoch, epoch)
        ...
        self.save_model_best_output(best_val_epoch)
