...
class BaseModel(torch.nn.Module):
    ...
    def networkx_to_torch3(self,networkx_graph ):
        graph = dhg.Hypergraph(networkx_graph)
        graph = graph.to(self.device)
        return graph
    ...
    def log_epoch(self, pbar, metrics, epoch, loss, epoch_time, logging_epochs, metric_name='loss'):
        ...
        if epoch and epoch % logging_epochs == 0:
            with open("loss.csv", "a+") as loss_f:
                loss_f.write(str(epoch) + "," + str(loss.cpu().detach().numpy()) + "\n")
            
            with open("macro.csv", "a+") as macro_f:
                macro_f.write("epoch_end " + str(epoch) + "\n")

            pbar.set_description(
                "Epoch {:05d} | Loss {:.3f} | Accuracy {:.3f}/{:.3f}/{:.3f} | Time {:.4f}".format(epoch, loss,
                                                                                              train_rmse,
                                                                                              val_rmse, test_rmse,
                                                                                              epoch_time))
    ...
    def evaluate_model(self, logits, target_labels, mask):
        ...
        with torch.no_grad():
            ...
            if self.task == 'regression':
                ...
                metrics['f1_ma'] = torch.Tensor([f1_score(y.cpu().numpy(), (pred.max(1)[1]).cpu().numpy(), average="macro")])

                with open("macro.csv", "a+") as macro_f:
                    macro_f.write(str(metrics['f1_ma'][0].cpu().detach().numpy()) + "\n")
    ...
    def save_model_output(self, logits, target_labels, val_mask, epoch):
        x = logits[val_mask]
        y = target_labels[val_mask]
        
        path = "./graph/" + config.get("dataset") + "/" + config.get("filename") + "/detail"
        if not os.path.exists(path):
            os.makedirs(path)

        filename = path + "/epoch_" + str(epoch)

        x = x.cpu().detach().numpy()
        y = y.cpu().detach().numpy()

        np.savetxt(filename + ".x", x)
        np.savetxt(filename + ".y", y)
    
    def save_model_best_output(self, epoch):
        path = "./graph/" + config.get("dataset") + "/" + config.get("filename")
        filename = "/detail/epoch_" + str(epoch)

        copyfile(path + filename + ".x", path + "/graph.x")
        copyfile(path + filename + ".y", path + "/graph.y")

        rmtree(path + "/detail")
    
    def train_and_evaluate(self, model_in, target_labels, train_mask, val_mask, test_mask,
                           optimizer, metrics, gnn_passes_per_epoch, epoch):
        loss = None
        ...
        self.save_model_output(h, target_labels, train_mask, epoch)
